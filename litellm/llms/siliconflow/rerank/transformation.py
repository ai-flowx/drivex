"""
Transformation logic from Cohere's /v1/rerank format to SiliconFlow's  `/v1/rerank` format.
"""
...
class SiliconFlowRerankConfig(BaseRerankConfig):
    ...
    def transform_rerank_response(
        ...
        should_return_documents = request_data.get("return_documents", True)
        original_documents = request_data.get("documents", [])
        ...
        for result in _results:
            ...
            if isinstance(document, dict):
                transformed_result["document"] = {
                    "text": document.get("text", "")
                }
            elif isinstance(document, str):
                transformed_result["document"] = {"text": document}
            elif should_return_documents:
                result_index = result.get("index", -1)
                if 0 <= result_index < len(original_documents):
                    original_document = original_documents[result_index]
                    if isinstance(original_document, str):
                        transformed_result["document"] = {"text": original_document}
                    elif isinstance(original_document, dict):
                        transformed_result["document"] = {
                            "text": original_document.get("text", "")
                        }
    ...
    def validate_environment(
        ...
        if api_key is None:
            raise ValueError(
                "api_key is required. Set via `api_key` parameter or `SILICONFLOW_API_KEY` environment variable."
            )
        return {
    ...
    def calculate_rerank_cost(
        ...
        """
        SiliconFlow reranker is priced at $0.000000018 per token.
        """
        if (
            model_info is None
